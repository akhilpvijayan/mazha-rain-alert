...
Usage:
  ...
  python ksdma_scraper.py [--debug]

---
IMPORTANT: this file intentionally does NOT include an "IMD 5-day forecast
block" merger, multi-date "&" line parsing, or a "use nearest upcoming
date if today is missing" fallback. Those additions appeared in a pasted
version of this file that I did not write and could not verify against
real captured data - and the "nearest upcoming date" fallback in
particular is a real bug: it shows a FUTURE day's forecast as if it were
today's actual status whenever today isn't in the bulletin, which is
exactly the wrong failure mode for a safety-relevant alert app. If you're
editing this file with another tool between sessions, please only bring
back additions that have been tested against real captured page text
(see debug workflow below) - untested "fixes" for problems that were
never actually observed have caused real regressions here more than once.

Bugs found and fixed against REAL captured data from the live page (each
one verified, not guessed):
...
4. Colors must be filtered to a specific date (defaults to today, strictly
   - no fallback to a different date). A 5-day forecast blended together,
   or silently substituted with a future date, would show today an alert
   color that isn't actually valid for today.

If build_district_colors() is ever wrong again, the fix belongs in
extract_with_regex() or the date-matching logic - always verified against
a real --debug dump first (see run()), never a speculative addition.
"""
...
# Includes the alternate spelling of Thrissur (തൃശ്ശൂർ, extra ശ) that
# KSDMA itself uses inconsistently across bulletins.
DISTRICT_MAP = {
...
def get_available_dates(data: dict) -> list:
    ...
    check this before assuming a fully-green (or any-colored) map reflects
...
def build_district_colors(data: dict, target_date: str | None = None) -> dict:
    ...
    STRICT date matching, by design: `data` can contain entries for
    several different dates at once (a multi-day forecast). This ONLY
    colors a district based on alerts whose `date` exactly matches
    `target_date` (defaults to today, IST, wall-clock - no substitution).
    If today isn't in the bulletin at all, every district correctly comes
    back "green" here - check get_available_dates()/has_data_for_today in
    the saved output to tell that apart from a confirmed-clear day.

    Do NOT change this to fall back to "the nearest upcoming date" if
    today is missing - that was tried and is a real bug: it silently
    shows a FUTURE day's forecast as if it were today's actual status,
    which is the wrong failure mode for a safety-relevant alert app.
    """
...
def run(debug: bool = False):
    ...
    available_dates = get_available_dates(validated)
    today_str = datetime.now(IST).strftime("%Y-%m-%d")
    ...
    if today_str not in available_dates:
        print(f"\n[info] NOTE: no bulletin entry for today ({today_str}). "
              f"Available dates in this bulletin: {available_dates}. "
              f"All-green above means 'no data for today', not 'confirmed clear'.")
